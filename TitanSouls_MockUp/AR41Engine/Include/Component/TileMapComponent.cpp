...
void CTileMapComponent::SetTileTexture(const std::string& Name, const TCHAR* FileName, const std::string& PathName)
{
...
void CTileMapComponent::SetTileTextureArray(const std::string& Name, const std::vector<const TCHAR*>& vecFileName, const std::string& PathName)
{
...
ETileOption CTileMapComponent::GetTileOption(const Vector3& Pos)
{
	int	Index = GetTileIndex(Pos);

	if (Index == -1)
		return ETileOption::End;

	return m_vecTile[Index]->GetTileOption();
}
...
void CTileMapComponent::ChangeTileOptionByIdx(int idx, ETileOption Option)
{
	CTile* Tile = GetTile(idx);
...
void CTileMapComponent::TestTileSetZFG()
{
...
void CTileMapComponent::CreateTextureFrameData(int countX, int countY, float sizeX, float sizeY)
{
	m_TextureXCount = countX;
	m_TextureYCount = countY;
	m_TextureXSize = sizeX;
	m_TextureYSize = sizeY;

	int TotalCount = m_TextureXCount * m_TextureYCount;

	for (int i = 0; i <= TotalCount; ++i)
	{
		Animation2DFrameData	Data;

		float StartX = i % m_TextureXCount * m_TextureXSize;
		float StartY = i / m_TextureYCount * m_TextureYSize;

		Data.Start = Vector2(StartX, StartY);
		Data.End = Vector2(StartX + m_TextureXSize, StartY + m_TextureYSize);

		m_vecTileFrame.push_back(Data);
	}

	m_TileStartFrame = Vector2(0.f, 0.f);
	m_TileEndFrame = Vector2(0.f + m_TextureXSize, 0.f + m_TextureXSize);
}

void CTileMapComponent::SetTileArrayData(const std::vector<unsigned int> TileStart)
{
	if (m_Count != TileStart.size())
		return;

	for (int idx = 0; idx < m_Count; idx++) {
		int temp = TileStart[idx];

		if (temp == 0) {
			m_vecTile[idx]->m_TileStart = Vector2(0.f, 0.f);
			m_vecTile[idx]->m_TileEnd = Vector2(0.f, 0.f);
			continue;
		}

		float xIdx = (temp % m_TextureXCount) * m_TextureXSize;
		float yIdx = (temp / m_TextureYCount) * m_TextureYSize;

		m_vecTile[idx]->m_TileStart = Vector2(xIdx, yIdx);
		m_vecTile[idx]->m_TileEnd = Vector2(xIdx + m_TextureXSize, yIdx + m_TextureYSize);
	}
}
...
bool CTileMapComponent::Init()
{
	...
	m_TileMapCBuffer->SetEnd(Vector2(1024.f, 1024.f));
	m_TileMapCBuffer->SetFrame(0);

	//SetTileMaterial("TileMap");


	//CreateTextureFrameData 작업 전 데이터

	//for (int i = 0; i <= 4096; ++i)
	//{
	//	Animation2DFrameData	Data;

	//	float StartX = i % 64 * 16.f;
	//	float StartY = i / 64 * 16.f ;

	//	Data.Start = Vector2(StartX, StartY);
	//	Data.End = Vector2(StartX + 16.f, StartY + 16.f);

	//	m_vecTileFrame.push_back(Data);
	//}

	//m_TileStartFrame = Vector2(32.f, 0.f);
	//m_TileEndFrame = Vector2(48.f, 16.f);
